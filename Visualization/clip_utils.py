...
def Delta(vec, M):
    if not M % 2 == 0:
        ...
        for i in range(N):
            delta[i] = (
                np.sum([m * (vec[M + i + m] - vec[M + i - m]) for m in range(1, M + 1)])
                / den
            )
...
files_check = [
    file[:-3] + "wav" for file in files_in if file[:-3] + "wav" not in files_out
]
...
def batch_training(main_dir, dataset_size=1000, delete=True, shuffle=True):
    ...
    while len(list_path) > dataset_size:
        ...
        if delete:
            ...
            temp_files = [
                os.path.join(data_dir, "temp_conv", file) for file in temp_files
            ]
