...
def listen_to_wrong_audio(
    ...
    if row[ind] > col[ind]:  # more predicted than true
        ...
        path_missclassified = data_frame[data_frame["category"] == cat_false].iloc[0][
            "full_path"
        ]
        ...
        if index_of_missclassified == ind:
            index_of_missclassified = np.argsort(np.max(confusion_mtx[ind, :], axis=0))[
                -2
            ]
        cat_false = labels[index_of_missclassified]
...
def visualize_the_weights(
    ...
    if layer_name is None:
        ...
        w0 = model.get_layer(index=layer_number).get_weights()[
            0
...
def plot_latent_space(
    ...
    if type(show_labels) == dict:
        ...
        show_labels = {
            **show_labels,
            **{k: 0 for k in range(10) if k not in show_labels.keys()},
        }  # out of the if because we want to add the 0 values also if the dict with numeric keys is incomplete
