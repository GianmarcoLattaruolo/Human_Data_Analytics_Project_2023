...
class MFCCWithDeltaLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        if self.order == 2:
            ...
            delta_delta_mfcc = (
                padded_delta_mfcc[:, 2:] - padded_delta_mfcc[:, :-2]
            ) / 2.0
...
def create_dataset(
    subfolder_path,  # folder of the audio data we want to import
    ...
    if labels is not None:
        if (
            ...
            if (
                ...
                and os.path.exists(save_val)
                and os.path.exists(save_labels)
            ):
                if verbose > 0:
            ...
            # tell if you have just some of train, val and test saved, not all
                ...
                os.path.exists(save_train)
                or os.path.exists(save_test)
                or os.path.exists(save_val)
                or os.path.exists(save_labels)
            ):
    ...
    @tf.autograph.experimental.do_not_convert
    def spectral_preprocessing_audio(
        audio,
        sample_rate=44100,
        segment=20,
...
def create_dataset_lite(
    ...
    if load_saved_dataset is not None:
        ...
        dataset = (
            dataset.cache(filename=cache_file)
            .shuffle(dataset.cardinality().numpy(), reshuffle_each_iteration=True)
            .batch(batch_size)
    ...
    if preprocessing is not None:
        ...
        if labeled:
            dataset = dataset.map(
                lambda audio, label: (
                    tf.py_function(
                        func=spectral_preprocessing,
                        inp=[preprocessing, audio],
                        Tout=tf.float32,
                    ),
                    label,
                ),
            ...
            if resize:
                ...
                dataset = dataset.map(
                    lambda audio, label: (
                        tf.py_function(func=reshape, inp=[audio], Tout=tf.float32),
                        label,
                    ),
                    num_parallel_calls=tf.data.AUTOTUNE,
                    deterministic=False,
    ...
    if labeled:
        dataset = dataset.map(
            lambda audio, label: (audio / max, label),
            num_parallel_calls=tf.data.AUTOTUNE,
            deterministic=False,
        )
    else:
        dataset = dataset.map(
            lambda audio: audio / max,
            num_parallel_calls=tf.data.AUTOTUNE,
            deterministic=False,
        )
...
def K_fold_training(
    ...
    model = KerasClassifier(
        ...
        epochs=epochs,
        verbose=verbose,
        callbacks=[callbacks_loss, callback_acc],
        **params,
    )
...
def compile_and_fit(
    ...
    if model_filename is not None:
        callbacks = [
            tf.keras.callbacks.EarlyStopping(verbose=verbose, patience=patience),
...
def count_parameters(model):
    ...
    non_trainable_params = sum(
        tf.keras.backend.count_params(p) for p in model.non_trainable_variables
    )
...
def build_tuner(
    build_model,  # A function that takes hyperparameters as inputs and returns a Keras model to be optimized.
...
def create_US_dataset(
    ...
    if preprocessing is not None:
        ...
        save_train_file = os.path.join(
            main_dir,
            "Saved_Datasets",
            "US_" + num(folder_number) + "_" + preprocessing + "_train_ndim3_resized",
        )
        save_test_file = os.path.join(
            main_dir,
            "Saved_Datasets",
            "US_" + num(folder_number) + "_" + preprocessing + "_test_ndim3_resized",
        )
        save_val_file = os.path.join(
            main_dir,
            "Saved_Datasets",
            "US_" + num(folder_number) + "_" + preprocessing + "_val_ndim3_resized",
        )
        normalize = True
    else:
    ...
    train, val, test, INPUT_DIM = create_dataset(
        path_to_ogg_files,
