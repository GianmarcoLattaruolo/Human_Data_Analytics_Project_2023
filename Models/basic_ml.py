...
def extract_flatten_MFCC(
    ...
    if (
        len(audio) > 200000
    ):  # it may happend that the audio is already transformed somehow...
...
def extract_labels(
    df,
):  # extract the category names from a dataframe (df_ESC10 or df_ESC50) in the correct
    # order fot the confusion matrix labels
    labels_name = {
        target: name for (target, name) in np.asarray(df[["target", "category"]])
    }
...
def basic_ML_experiments_gridsearch(
    dataset,  # numpy array with the raw audio loaded
    ...
    if dataset.shape[0] == 2000:
        ...
        original = {
            ...
            40,
            41,
        }  # the orginal 10 target classes in ESC10
        if classes == original:
            name_df = "ESC10"
        else:
            name_df = "random10"
    ...
    mfcc_args_list = [
        {"mfcc": a, "n": b} for a in values_mfcc["mfcc"] for b in values_mfcc["n"]
    ]
    ...
    indicators = [logistic_regression, SVM, decision_tree, random_forest, KNN]
    ...
    if logistic_regression:
        ...
        if MFCC:
            ...
            X_train_mfcc = np.apply_along_axis(
                lambda x: extract_flatten_MFCC(audio=x, cepstral_num=mfcc, N_filters=n),
                1,
                X_train,
            )
            ...
            pipe = make_pipeline(
                StandardScaler(),
                LogisticRegression(C=0.1, fit_intercept=True, max_iter=1000),
            )
