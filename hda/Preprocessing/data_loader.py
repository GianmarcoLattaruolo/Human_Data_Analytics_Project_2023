...
def load_metadata(
    ...
    if ESC_US:
        file_path = os.path.join(
            main_dir, "Data", "meta", "ESC-US.csv"
        )  # this csv file is useless since has no reference to the files
        ...
        for i, folder in enumerate(os.listdir(ESC_US_paths)):
            ...
            d = pd.DataFrame(
                (files, full_path_files), index=["filename", "full_path"]
            ).transpose()
        ...
        for category_folder in list(set(df.category)):
            ...
            for old_path in df.full_path[df.category == category_folder]:
                new_path = os.path.join(
                    data_dir, category_folder, old_path.split("\\")[-1]
                )
